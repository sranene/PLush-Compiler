...
function calculateSimilarity(val map_1 : [[int]], val map_2 : [[int]]) : float {
    ...
    var matchingElements : float := .0;
    
    var i : int := 0;
    while i < length(map_1) {
        var j : int := 0;
        while j < 4 {
            totalElements := totalElements + 1.0;
            if map_1[i][j] = map_2[i][j] {
                matchingElements := matchingElements + 1.0;
            }
            j := j + 1;
        }
        i := i + 1;
    ...
    val percentage : float := (matchingElements / totalElements) * 100.0;
